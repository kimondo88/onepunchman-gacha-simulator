...
export default function SpiritHallComplete(){
    ...
    const [copies, setCopies] = useState(23);
    ...
    function count(value: string){
        setCopies(parseInt(data.completeshcopies[value]));
    }
    ...
    function calcOutput(){
        let max = parseInt(data.completeshcopies["sh6"]);
    ...
    return (
        <div>
            <div>Complete Spirit Hall Copies</div>
            ...
            <input type="number" id="havecopies" min={0} max={20}
                onChange={(e) => {
